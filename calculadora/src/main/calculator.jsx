...
export default class Calculator extends Component{
    ...
    setOperation(operation){
        ...
        if(this.state.current === 0){
            ...
            switch(currentOperation){
                ...
                case '/': values[0] /= values[1]; break;
                case '*': values[0] *= values[1]; break;
                default: console.log(values[0], values[1])
            }
    ...
    addDigit(n){
        ...
        try{if(n === '.' && this.state.displayValue.includes('.')){
            return
        }}catch(e){}
    ...
    formatNumber(number) {
        ...
        if (Math.abs(number) >= limit) {
          return scientificNotation.format(number);
        } else {
    ...
    render(){
        const formattedDisplayValue = this.formatNumber(this.state.displayValue);
        return (
